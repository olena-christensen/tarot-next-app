...
- [ ] **Pricing shows FREE as the active plan for everyone.** `SubscriptionPlans` hardcodes the FREE card's button to "Current plan" and never reads the user's real plan, so a MONTHLY/YEARLY subscriber sees Free marked active (mismatches the profile, which shows the true plan). Fix: fetch `GET /api/user/plan` on mount, mark the matching tier as current + disabled, keep SINGLE always purchasable; add an `includedBtn` ("Included") string in all 5 locales for lower tiers under the active one.
- [ ] **Reader modal — current-marker / paid-star collide.** In `ReaderSelection`, the current-reader dot (`&__card--current::after`) and the paid-tier ★ (`&__card--locked::after`) are both drawn on the same `::after` of the card, so a reader that is both current and locked renders them mashed together. Fix: make one of them a real element (e.g. a `__lock-badge` span) so they're independent. (User also wanted the current marker restyled as a bead seated on the top border — that's a design call, capture it when redoing.)
