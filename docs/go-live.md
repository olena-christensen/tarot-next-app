...
- [x] **Reconcile sweep couldn't recover payments older than 7 days** — fixed 2026-08-02. Two bugs compounded: the sweep's window was 30 min – 7 days (anything older orphaned forever), and `applyMonoInvoiceStatus` had no branch for mono's **`expired`** status, so an abandoned checkout fell through to the "unknown status" no-downgrade path and stayed `created` for good, holding `pendingPlanId` with it. `expired` is now terminal (clears `pendingPlanId`, records the ledger status, **no** dunning email — nothing failed, the user simply walked away), and the window is 90 days. The sweep can't grow unbounded because a resolved row drops out of the `created`/`processing` filter. Live example that surfaced it — `dantemariya@gmail.com`, a €39 YEARLY invoice created 28 Jun 17:22 UTC, one minute after a Google sign-up, never paid; mono had it as `expired`, we still said `created`. Cleaned by running the fixed path against it: `created`/`YEARLY` → `expired`/`null`, plan untouched at FREE, account intact.
