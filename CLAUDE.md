...
- **Who spoke it** (migration `add_reading_reader`, 2026-08-02): `Reading.readerId` is captured by `/api/readings/consume` from `state.selectedReader` and shown beside the date as `date · Reader`. The value is **validated against `READER_IDS` server-side** and stored as null if unknown — never trust the client's string, and never reject a reading the user has already seen over it. Null on every row written before this migration, and on anything whose reader can't be resolved; the UI shows nothing rather than guessing a name.
