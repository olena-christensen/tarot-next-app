...
- **Z-index layer scale (keep separated, do NOT collapse to equal values):** content ≤2 → `.main-header` 30 → `.header-greeting--bar` 31 → `.tarot-modal` (full-screen reading screen) 40 → `.main-footer--overlay` 50 → `.cookie-banner` 60 → `.loader` 70 → `.modal` 100 → `.reader-selection__summon-pane` (mobile bottom sheet, portaled to `body`) 110 → `.language-switcher` dropdown 200. `Modal.tsx` **portals to `document.body`** so popups escape parent stacking contexts (especially the fixed `.tarot-modal`) and layer globally.
...
- **Mono's `expired` status is terminal and must be handled.** An invoice the user never completed expires on mono's side. `applyMonoInvoiceStatus` clears `pendingPlanId`, records `paymentStatus: "expired"` and sends **no** dunning email (nothing failed — they abandoned checkout). Before 2026-08-02 there was no branch for it, so such rows fell through to the unknown-status no-downgrade path and stayed `created` forever, permanently "in flight". The reconcile sweep's window is **30 min – 90 days** (was 7 days, which orphaned anything older); it stays bounded because terminal rows drop out of the `created`/`processing` filter.
...
- **Sharing** (`Reading.shareId`, nullable + unique, migration `add_reading_deck_and_share`): the share button mints an unguessable token (9 random bytes, base64url) via `POST /api/readings/[id]/share` and publishes the reading at `/[locale]/r/[shareId]`. **The link IS the credential** — public, no account. Minting is **idempotent** (re-sharing returns the existing link, so a user can't accumulate live URLs they've forgotten); `DELETE` on the same route clears `shareId` and the page 404s immediately.
  - The public page is **anonymous by design**: cards, reading text, reader and date — **never** the owner's name. Don't add it.
  - `robots: noindex` — personal content, and thousands of thin auto-generated pages would drag the site's SEO down. OG/Twitter tags are still emitted so links unfurl; the description is the reading's **first sentence only**, so a preview intrigues without spilling the whole thing.
  - The share dialog uses **plain intent URLs** for X / Facebook / Telegram / WhatsApp — no third-party SDKs, so no trackers load and nothing touches cookie consent. `navigator.share` is offered as an extra where supported, resolved in an effect (reading it during render would desync SSR).
  - **OG image** (`opengraph-image.tsx` in the same segment, built 2026-08-02): `ImageResponse` from `next/og` renders 1200×630 — title, reader, the three cards in the deck they were drawn with, and the wordmark, on the app's dark/gold gradient. **`runtime = "nodejs"`** (not the edge default) because it touches Prisma and `node:fs`. **Two Vercel filesystem facts drove its shape** (both surfaced as a hard 500 in production while local dev was perfectly happy — 2026-08-02): (a) `public/` is served by the CDN and is **NOT** on the serverless function's filesystem, so card art is **fetched over HTTP** from `NEXT_PUBLIC_APP_URL`, never `readFile`d — and at 76MB it's far too big to bundle anyway; (b) anything read via a runtime `readFile(join(process.cwd(), …))` is invisible to Next's tracer, so the font needs an explicit `experimental.outputFileTracingIncludes` entry in `next.config.mjs` keyed by the route. The font load is also wrapped so a missing file degrades to the default typeface instead of 500ing the preview. **Satori cannot decode WebP** — handing it one throws `a is not iterable` and kills the entire response (an empty reply, not a 500), so each card is transcoded to PNG with `sharp` (already a direct dependency) at 2× the drawn size. Any future art added to an OG image must go through the same transcode. Satori can't resolve CSS custom properties — the palette hexes are repeated in that file and must be kept in step with `_variables.scss`. The font is `assets/fonts/Raleway-Light.ttf`, committed deliberately: `next/font/google` only produces hashed woff2 inside `.next`, which isn't readable at runtime. `params` is a plain object here (Next 14), not the promise the current Next docs show.
- **Provenance — who spoke it, which deck** (migrations `add_reading_reader`, `add_reading_deck_and_share`): `Reading.readerId` and `Reading.deckId` are captured by `/api/readings/consume` at draw time and shown beside the date as `date · Reader`; card art renders from the stored deck, so a reading keeps the deck it was drawn with even after the user switches. Falls back to `DEFAULT_DECK` when null. The value is **validated against `READER_IDS` server-side** and stored as null if unknown — never trust the client's string, and never reject a reading the user has already seen over it. Null on every row written before this migration, and on anything whose reader can't be resolved; the UI shows nothing rather than guessing a name.
- **Favourites & notes** (migration `add_reading_note_and_favorite`, 2026-08-02): `Reading.note` (nullable text, ≤2000 chars) and `Reading.isFavorite` (boolean). The star toggles optimistically and rolls back on a failed write; un-starring while the filter is on removes the row from view. `?favorites=1` on `GET /api/readings` does the filtering **server-side** so cursor paging stays correct — never filter an already-paged list client-side. The note renders under the reading text as a quoted aside and is deliberately kept on the printed sheet (it's the user's own writing). Empty-with-filter is a distinct state from empty-ledger: it offers "show all", not "draw your first fate".
- **`PATCH /api/readings/[id]` is partial.** Only keys present in the body are written (`title`, `note`, `isFavorite`), so the star can toggle without clearing a note. Absent key = untouched; empty string = cleared to null.
- **One editor for title + note.** The pencil opens "Edit This Reading" holding both fields — you name a reading to find it and annotate it to remember why, so splitting them into two modals would be busywork.
...
- **The current deck is marked by the gold bead, not a badge (2026-08-04).** Same `radial-gradient(circle at 35% 30%, gold, primary)` + inset-shadow jewel as the current reader and the current plan — one "this is the chosen one" marker app-wide. The old `SELECTED` pill and its `&__badge` style are gone; the disabled CTA still says it in words.
- **Preview art is sized by HEIGHT, never width.** The three decks' Fool cards have three different aspect ratios (0.57 / 0.63 / 0.58), so a fixed width guaranteed three different heights and the row read as ragged. At `md`+ the grid uses `repeat(3, auto)` + `justify-content: center` so each frame hugs its own art; card heights still match because grid items stretch to the row.
- **`.decks__card` carries a faint gold wash**, not near-black. Each deck's art has its own dark border baked into the image; against a dark card those borders vanished and the cards read as different sizes even once the heights matched.
...
- `.header-avatar` carries intrinsic styles only (size, border, radius) — the header owns placement, including `flex: 0 0 auto` so a long greeting can never squeeze the circle.
- **The rotating greeting is rendered TWICE (2026-08-04), one visible per breakpoint.** `HeaderGreeting` is plain text, not a link — the avatar is the only route to `/profile`, so the greeting no longer duplicates that target. Below `md` it is a **fixed strip pinned to the top of the viewport** (`.header-greeting--bar`, `z-index: 31`); at `md`+ that strip is `display: none` and the copy shows inline in the nav row instead (the `<li>` is what hides — a class on the anchor loses to `.main-menu .main-menu__link` on specificity). `display: none` also drops the hidden one from the accessibility tree, so nothing is announced twice, and two instances agree because `resolveGreeting` caches its KEY in sessionStorage.
- **The strip MUST stay a sibling of `<header>`, never a child.** `.main-header` is animated with a `transform`, which makes it the containing block for any `position: fixed` descendant — the same trap that forced the reader-selection bottom sheet to portal out. The header reserves room for it via a `--greeted` modifier (not a blanket rule) so signed-out visitors don't get dead space.
- **Why it exists:** the greetings are whole sentences, and the old `max-width: 45vw` + ellipsis rule (written for a long *name*) clipped them to a few unreadable words on a phone. Ellipsis is the wrong tool for a sentence.
...
- **Field-row pattern (rebuilt 2026-08-04): the value IS the control.** Every field is a `.user-profile__field--row` (label left, value right). **The pencil icons are gone** — each row's value is a `.user-profile__value-btn`, a naked button styled to match `.user-profile__value` exactly (no background, border or underline; pointer cursor + hover colour are the whole affordance) which opens that row's editor. Don't reintroduce `__edit-icon` on a row. Rows: Avatar, Name, Email, Station, Reading Credits, Deck, Reader, Ledger of Fates, The Daily Whisper, The Nudge, Language, Password.
- **Two rows are controls rather than values:** The Daily Whisper and The Nudge use `<Switch>` (see Switch §), which carries the state word *inside* the track — no separate value span beside it.
- **Email is masked** behind a fixed twelve-dot `EMAIL_MASK` with an eye toggle (`eye.svg` → `eye-off.svg` once revealed). Fixed length on purpose: dots matching the real address leak how long it is. This is the one icon button still allowed in a row.
- **`.user-profile__value-group` sets `margin-left: auto`.** `space-between` only pins the value right while both parts share a line; once a long label wraps the row, the value lands alone on line two and would sit under the label.
- **Actions at the foot are stacked, not inline:** `<MysticButton>` "Back to the Sanctum" (the same component the reading screen uses) on top, with a deliberately half-scale `.user-profile__btn` sign-out beneath it.
...
## Sign-in ("remember me" and autofill)

- **`ui.rememberMe` ("Keep me past the veil") is a checkbox on the sign-in form only**, ticked by default. On sign-up it is hidden — a brand-new account keeping its session is the sane default. It does **two** things:
  1. **Session length.** `session.maxAge` is static config and cannot vary per sign-in, so the choice rides on the token: `authorize` reads `credentials.rememberMe` (a STRING over that transport) and the `jwt` callback stamps `token.sessionExpiresAt = now + SHORT_SESSION_MS` (12 h) when unticked, `null` when ticked. Every session read checks that deadline and **throws** past it — the same eviction mechanism as the deleted-user check, which NextAuth v4 catches to clear the cookie. The check runs BEFORE the refresh logic so an expired token can't be renewed back to life. Google sign-in has no checkbox to carry, so it is always remembered.
...
- **Two axes, and they catch different attacks.** By **subject** (`login:email:…`) catches a targeted brute force spread across many IPs, which an IP limit cannot see. By **IP** (`login:ip:…`, `register:ip:…`, `contact:ip:…`) catches volume, including attempts on addresses that don't exist. Keying on the email of a non-existent account is deliberate — refusing to count it would make "did a row appear" an enumeration oracle.
- **The check sits IN FRONT of bcrypt in `authorize`.** The hash is cost 12 (~250ms CPU) on purpose, so an unthrottled flood reaching it *is* the denial of service. Order matters; don't move it below the lookup.
- **`decideRateLimit` is pure and tested** (11 tests) — the policy is the part worth being sure about. Notably: attempts made *while blocked* do not increment the count, or a flood would keep extending its own release.
- **It fails OPEN.** A database blip returns "not blocked" rather than locking everyone out; a throttle that becomes an outage is worse than the attack. Any change here must keep that.
- **A successful sign-in clears the email counter** (`clearRateLimit`) — four typos then the right phrase shouldn't leave someone one slip from a lockout.
- **The login form distinguishes the two failures.** `authorize` throws `rate_limited`; anything else is a real credential failure. Showing "wrong password" for a lockout sends people off resetting a password that was never wrong. String: `ui.tooManyAttempts`.
- Current policies: login 10/15 min per email, 30/15 min per IP; register and contact 5/hour per IP. Stale rows are pruned by the hourly reconcile sweep.
- **Verified on production 2026-08-04.** Attempts 1–10 rejected normally, 11 and 12 rate-limited; the stored count stopped at 11 (attempts made while blocked don't increment), and the IP counter kept climbing independently while the email axis was blocked. Probe used a throwaway `@example.invalid` address — the limiter counts before the user lookup, so a real account is never needed to test it. NOT covered: `clearRateLimit` on a successful sign-in, and the register/contact limits (5/hour would lock the tester's own IP out for an hour).
- **This is the app-level layer only.** Vercel's Firewall sits in front of it and is dashboard configuration — the two are complements, not alternatives. Three `rate_limit` rules exist on production (sign-in 20/60s, register and contact 10/60s, all keyed by IP), staged with the **Log** action so they record without blocking. **Vercel does not bill for WAF-blocked traffic**, which is the real argument for the edge layer: every request the app-level limiter refuses has already cost an invocation and a query. Automatic DDoS mitigation is separate, always on, and free on every plan.

## Switch component

- `src/components/Switch.tsx` + `_switch.scss`. `role="switch"` with `aria-checked`, no Save step — it commits immediately. Used by the Daily Whisper and Nudge profile rows.
- **The accessible name is the THING being toggled** ("The Daily Whisper"), never the action ("Silence it") — state is already carried by `aria-checked`, so an action-shaped label contradicts it.
- **Both state words are always rendered, stacked in one grid cell** (`grid-area: 1 / 1`), only the active one visible. The track therefore always reserves the WIDER of the two, so flipping it can't change its width and shove the row sideways — the same anti-reflow trick as the reader selection, and what makes it locale-proof without magic numbers.
- The knob moves by `order`, not a translate, so padding stays symmetrical at both ends. It reuses the app's existing gold-jewel vocabulary (`radial-gradient(circle at 35% 30%, gold, primary)` + inset shadow) with a small `switchJewelPulse`, disabled under `prefers-reduced-motion`.

## Cursors on disabled controls

- **`cursor: not-allowed` is banned on anything whose disable is transient** — a form submitting, a preference saving. It flashed a ban icon under the pointer on every click and read as rejection for requests that were succeeding. Removed from `.btn`, `.mystic-btn`, `.switch` and the profile's icons/CTAs; the dim opacity is the only busy signal.
- It is still correct where a control is genuinely unavailable rather than busy: `.decks__cta` and `.subscription__cta` when they mean "this is already your deck / your current plan".

## Form checkboxes

- `.form__checkbox` (in `_form.scss`) styles the native input via `appearance: none`. Before 2026-08-04 there were **no checkbox styles at all** — the terms, 18+ and remember-me boxes were raw grey system controls on a gold-on-black form.
- **Empty box in both states, no fill.** Unchecked is a dim outline with a faint glow; checked brightens the border to `--gold` and adds a tick drawn from two borders (no icon font, no SVG). The tick's glow is a `drop-shadow` **filter**, not a `box-shadow` — a box-shadow would halo the invisible rectangle around the tick instead of its shape.
